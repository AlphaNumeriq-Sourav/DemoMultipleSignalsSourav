    ...
    for i in range(len(Choices)):
        ...
        if condition:
            ...
            if variable == 'Trail':
                ...
                logger.info(
                    f'Entry Long at {time1} for SignalID : {Signal_uni_name}  at EntryPrice : {Price}  StopLoss : {StopLoss} Target : {TP_val} Lotsize : {LotSize} ATR Value : {ATR} OrderComment : {order.comment} Flag : {flag} OrderID : {order_id}')
                SendSkypeNotification(
                    f'Entry Long at {time1} for SignalID : {Signal_uni_name}  at EntryPrice : {Price}  StopLoss : {StopLoss} Target : {TP_val} Lotsize : {LotSize} ATR Value : {ATR} OrderComment : {order.comment} Flag : {flag} OrderID : {order_id}', skype_connect)
