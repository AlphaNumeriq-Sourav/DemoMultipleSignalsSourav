...
# -------------------------x-----------------------x---------------------------x---------------------------x---------------------------x---------------------------
# Function to Get H4 Forex Signals for GBPUSD
# Signal 1 = high[0] > low[7]; high[2] > open[4]; low[3] <= open[5]; close[4] <= open[6]; ValueOpen(5)[1] > ValueClose(5)[5]; ValueClose(5)[3] <= ValueOpen(5)[4]
# Signal 2 = low[0] > open[6]; open[0] > close[2]; low[1] > close[5]; high[8] > close[9]; low[7] <= open[9]; CompositeATR(2;24)[0] > CompositeATR(2;24)[2]
# -------------------------x-----------------------x---------------------------x---------------------------x---------------------------x---------------------------
...
def entry_signal1(data,choice,index):
    ...
    if choice == 1:
        ...
        condition = (data.iloc[index-0]['low'] > data.iloc[index-6]['open']) and \
                    ...
                    (data.iloc[index-0]['CompositeATR'] > data.iloc[index-2]['CompositeATR']) 
        variable = "Trail"
...
# -------------------------x-----------------------x---------------------------x---------------------------x---------------------------x---------------------------
# Function to Execute the Trades of GBPUSD when a New signal comes from signal 1 and also to Exit the Trades Also
# -------------------------x-----------------------x---------------------------x---------------------------x---------------------------x---------------------------
